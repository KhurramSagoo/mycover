...
const IndividualPaperless = () => {
  const cardData = [
    ...
    },
    {
      ...
      detail:
        "Make Claims in 15 mins! Take a picture, estimate damage and get paid!",
      image: task,
      dot: null,
    },
  ];
  return (
    <div className=" bg-[#f3faf8]  h-full lg:p-16 md:p-12 px-5">
      ...
      <div className=" py-4 sm:py-6 md:py-8 ">
        <p className=" md:text-5xl leading-snug  text-secondary text-2xl font-bold text-center">
          Skip The Hassle & Paper Work!
        </p>
        <br />
        <p className=" md:text-[48px]  text-xl font-bold text-center text-primary">
      ...
      <div className="  mt-14 pt-12 flex items-start lg:justify-between  md:justify-between justify-start w-full flex-col sm:flex-row flex-wrap">
        {cardData.map((item, index) => (
          <div
            key={index}
            className=" flex items-start md:justify-start justify-center flex-col  p-5 md:p-2 lg:p-0 sm:w-[300px] w-full"
          >
            <div className=" flex items-end justify-center">
              ...
              <img
                loading="lazy"
                src={item.dot && item.dot}
                className="hidden lg:flex "
                alt=""
              />
            </div>
            <div className=" mt-6">
              <p className="text-medium font-bold text-secondary break-words">
                {item.title}
              </p>
