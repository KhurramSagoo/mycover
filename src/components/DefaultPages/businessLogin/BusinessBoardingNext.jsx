...
const BusinessBoardingNext = () => {
  ...
  const [selected, setSelected] = useState("Yes");
  const [tabId, setTabId] = useState(0); // Default tab ID
  const [newIdData, setNewIdData] = useState(0);
  const [getParam, setGetParam] = useState("");

  const getDataById = () => {
    if (tabId) {
      setNewIdData(tabData.find((item) => item.id === tabId));
    } else {
      console.log(tabId);
    }
  };

  // console.log(newIdData, tabId);

  useEffect(() => {
    const storedTabId = localStorage.getItem("selectedProductId");
    const productParam = localStorage.getItem("productParam");
    const productId = localStorage.getItem("productId");

    console.log(typeof tabId, tabId, getParam);

    // const availableId = JSON.parse(storedTabId);
    // console.log(typeof availableId);
    if (storedTabId) {
      setTabId(JSON.parse(storedTabId));
    }
    if (productParam) {
      setGetParam(productParam);
    }
          ...
          <div className=" flex items-center justify-center">
            <div className="mt-6 py-4 px-6 max-w-[565px] w-full flex items-center justify-between bg-[#ebe9fe]">
              ...
              <div className="bg-[#d9d6fe] px-2 py-2 rounded-lg text-sm flex items-center justify-center text-white">
                <button
                  className={`px-1  ${
                    selected === "Yes" ? "bg-[#3C1E96]  rounded" : " text-black"
                  }`}
